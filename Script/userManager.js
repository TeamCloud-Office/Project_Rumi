...
(function () {
  function UserManager() {
    ...
    return {
      ...
      Purchase: function (id, count, itemName) { //구매
        let user = Find(id);
        if (user === null) return "생성된 계정이 없어요.";

        let targetItem = DataBase.Search(type, "itemName", itemName)
        if (!targetItem) return "존재하지 않는 아이템이에요.";
        if (user.addRune(-(Number(targetItem["coin"]) * count))) {
          Save();
      ...
      Sell: function (id, count, itemName) { //판매
        let user = Find(id);
        if (user === null) return "생성된 계정이 없어요.";

        let itemList = user.getItemList("all"); //{type, name, count}

        let sellItem = DataBase.Search(type, "itemName", itemName) ? DataBase.Search(type, "itemName", itemName) : null;

        if (sellItem === null) return `해당 아이템을 찾을 수 없어요.`

        if (user.isItemCount(itemType, sellItem["name"], count)) {
          let sellPrice = Math.floor(Number(sellItem["coin"]) * 2 / 3);
          if (sellPrice === 0) sellPrice = 1;
          user.addCoin(sellPrice * count);
          user.removeItem(itemType, sellItem["name"], count);
          Save();
          return `원가: ${sellItem["coin"]}스타의 2/3 가겨인 ${sellPrice}스타에 판매할게요.`;
        } else {
          return `해당 아이템을 판매할 수 있는 개수가 없어요.`
        }
      },
      ...
      Eat: function (id, foodName) {
        let user = Find(id);
        if (user === null) return `생성된 계정이 없어요`;
        if (user.coin < DataBase.Find("FoodItem", "itemName", foodName).coin) return `재화가 부족해요.`;
        user.state.hunger -= DataBase.Find("FoodItem", "itemName", foodName).plus;
        return `식사료는 ${DataBase.Find("FoodItem", "itemName", foodName).coin}스타에요. 배고픔이 ${DataBase.Find("FoodItem", "itemName", foodName).plus}만큼 회복되요.`
      },
